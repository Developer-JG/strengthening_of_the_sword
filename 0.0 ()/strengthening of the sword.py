...
# 방지권
sword_6 = sword("+6 양날 검", 2000, 3500, 90, 1, "", 0, item_01, randint(1, 3))
sword_7 = sword("+7 심판자의 대검", 2000, 6100, 90, 1, "", 0, item_01, randint(2, 4))
sword_8 = sword("+8 마력의 검", 3000, 10000, 85, 1, "", 0, item_01, randint(3, 5))
sword_9 = sword("+9 타우 스워드", 5000, 20000, 80, 1, "", 0, item_02, randint(1, 3))
sword_10 = sword("+10 형광 검", 10900, 35100, 80, 1, "", 0, item_03, randint(1, 3))
sword_11 = sword("+11 피묻은 검", 20000, 160000, 75, 1, "", 0, item_04, randint(1, 3))
sword_12 = sword("+12 화염의 쌍검", 35000, 350000, 70, 1, "", 0, item_04, randint(2, 4))
sword_13 = sword("+13 불꽃 마검", 5000, 20000, 80, 2, "", 0, item_05, randint(1, 3))
sword_14 = sword("+14 마검 아포피스", 100000, 3000000, 65, 3, "", 0, item_06, randint(1, 3))
sword_15 = sword("+15 데몬 배틀 엑스", 180000, 7500000, 60, 4, "", 0, item_07, randint(1, 3))
sword_16 = sword("+16 투명 검", 300000, 14200000, 60, 7, "", 0, item_08, randint(1, 3))
...
def drop_item(player_1, repair):
    if player_1.sword.drop_item_count != 0:
        player_1.inventory["item"][player_1.inventory["item"].index(player_1.sword.drop_item)] = \
        player_1.inventory["item"][player_1.inventory["item"].index(player_1.sword.drop_item)].count + player_1.sword.drop_item_count
        if repair == "no":
            print(f"강화가 실패하여 '{player_1.sword.name}' 이/가 파괴되었습니다. '{player_1.sword.drop_item.name}' 을/를 {player_1.sword.drop_item_count} 개 주웠습니다.\n")
            player_1.sword = sword_0
        elif repair == "yes":
            print(f"'{player_1.sword.name}' 을/를 성공적으로 복구하였습니다. '{player_1.sword.drop_item.name}' 을/를 {player_1.sword.drop_item_count} 개 주웠습니다.\n")
    else:
        if repair == "no":
            print(f"강화가 실패하여 '{player_1.sword.name}' 이 파괴되었습니다.\n")
            player_1.sword = sword_0
...
# 판매
def sale(player_1):
    print(f"'{player_1.sword.name}' 이/가 성공적으로 판매되었습니다.\n")
