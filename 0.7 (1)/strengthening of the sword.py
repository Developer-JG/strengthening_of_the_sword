...
def drop_item(player_1, repair):
    if player_1.sword.drop_item_count != 0:
        ...
        player_1.inventory["item"][player_1.inventory["item"].index(player_1.sword.drop_item)] + player_1.sword.drop_item_count
        if repair == "no":
            print(f"강화가 실패하여 검이 파괴되었습니다.\n({player_1.sword.drop_item}) 을/를 {player_1.sword.drop_item_count} 개 주웠습니다.")
            player_1.sword = sword_0
        elif repair == "yes":
            print(f"검을 성공적으로 복구하였습니다.\n({player_1.sword.drop_item}) 을/를 {player_1.sword.drop_item_count} 개 주웠습니다.")
    else:
        if repair == "no":
...
def main():
    ...
    player_1 = player(sword_25, 1000000, 100, [])
