...
item_22 = common_item("+21 템페스트 골드", 0)
item_23 = common_item("+22 샤프 워커", 0)
...
item_list_3 = [item_21, item_22, item_23]
...
def inventory(player_1):
    ...
    n = 3
    divide_list = [player_1.inventory["item"][i * n:(i + 1) * n] for i in range((len(player_1.inventory["item"]) + n - 1) // n)]

    while True:
        print("\n")
        print("{0:=^25}".format(" 인벤토리 "))
        if plag == 1:
            for i in range(len(divide_list[0])):
                print("({0}) X {1}".format(player_1.inventory["item"][i].name, player_1.inventory["item"][i].count))
        elif plag == 2:
            for i in range(len(divide_list[1])):
                print("({0}) X {1}".format(player_1.inventory["item"][i].name, player_1.inventory["item"][i].count))
        elif plag == 3:
            for i in range(len(divide_list[2])):
                print("({0}) X {1}".format(player_1.inventory["item"][i].name, player_1.inventory["item"][i].count))
            difference = n - len(divide_list[2])
            while difference > 0:
