...
def drop_item(player_1, repair):
    if player_1.sword.drop_item_count != 0:
        ...
        if repair == "no":
            print(f"강화가 실패하여 '{player_1.sword.name}' 이/가 파괴되었습니다. '{player_1.sword.drop_item.name}' 을/를 {drop_item_count} 개 주웠습니다.\n")
            player_1.sword = sword_0
        elif repair == "yes":
            print(f"'{player_1.sword.name}' 을/를 성공적으로 복구하였습니다. '{player_1.sword.drop_item.name}' 을/를 {drop_item_count} 개 주웠습니다.\n")
    else:
        if repair == "no":
